...
public class ServiceGerenciadorFinanceiro {
	...
	public Divida getDivida(List<Divida> listaDeDividas, long id_divida) {
		if (!listaDeDividas.isEmpty()) {
			if (listaDeDividas.size() <= id_divida) {
			...
			throw new OperacaoInvalidaException("Lista vazia");
		}
	}

	// calcula o juros a partir do saldo
	...
	// dado uma divida e a data do pagamento calcula o valor da nova divida
	//Obs: ovalor é calculado a partir do ultimo estado da dívida;
	...
	// devolve o saldo atualizado entre o momento atual da dívida e a data dopagamento
	...
	// devolde o valor do juros pago entre o saldo atual da divida e o pagamento daparcela
	...
	// dada uma dívida, se quitada, devolve a data de quitação,caso contrario,devolve nulo;
